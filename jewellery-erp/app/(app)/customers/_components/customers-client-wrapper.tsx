...
import { useCreateCustomer } from "@/lib/query/hooks/use-customers";
...
export function CustomersClientWrapper({
  tenantId,
  initialCustomers,
}: CustomersClientWrapperProps): React.JSX.Element {
  ...
  const { data: customers, isLoading } = useQuery({
    queryKey: qk.customers.list(tenantId, searchParam),
    queryFn: async () => {
      const res = await customerApi.getCustomers({ search: searchParam });
      // Map to SerializedCustomer
  ...
  const { mutate: createCustomer, isPending: creating } = useCreateCustomer(tenantId);
