...
export default function SuppliersPage() {
  ...
  // Queries & Mutations
  const { data: suppliers, isLoading } = useSuppliers(tId, { search: searchQuery });
  const { mutate: createSupplier, isPending: creating } = useCreateSupplier(tId);
                ...
                {suppliers.map((sup) => (
                  <TableRow key={sup.id} className="hover:bg-muted/30">
                    ...
                    <TableCell className="text-right font-mono tabular-nums text-destructive">
                      {formatINR(sup.openingBalance.toString())}
                    </TableCell>
